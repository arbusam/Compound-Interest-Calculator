...
export default function Home() {
  ...
  const [useCustomRegularDeposits, setUseCustomRegularDeposits] = useState(false);

  const [useRegularDepositEndDateChange, setUseRegularDepositEndDateChange] = useState(false);

  const [annualPromotionRate, setAnnualPromotionRate] = useState("");
  const [regularDepositEndDate, setRegularDepositEndDate] = useState<Date>(new Date());
  ...
  const handleUseCustomRegularDepositsChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    setUseCustomRegularDeposits(event.target.checked);
  }

  const handleAnnualPromotionRateChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    let value = event.target.value;
  ...
  const handleAnnualPromotionRateBlur = (event: React.FocusEvent<HTMLInputElement>) => {
    let value = event.target.value;
  ...
  const handleUseRegularDepositEndDateChange = (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      {advancedOptionsOpen && (
        <div className="flex flex-col mt-5 w-1/3">
          <div className="flex items-center">
            <input id="checkbox" type="checkbox" value="" checked={useCustomRegularDeposits} onChange={handleUseCustomRegularDepositsChange} className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" />
            <label htmlFor="checkbox" className="ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">Use Custom Regular Deposits</label>
          </div>
          {useCustomRegularDeposits && (
            <div>
              <div className="flex flex-col items-left mt-5 ml-1">
                <label htmlFor="annualPromotionRate" className="block mb-2 text-md font-semibold">Annual Promotion Rate</label>
                <input id="annualPromotionRate" type="text" value={annualPromotionRate} onChange={handleAnnualPromotionRateChange} onBlur={handleAnnualPromotionRateBlur} className="block p-2 h-10 text-gray-900 border border-gray-300 rounded-lg bg-gray-50 text-xs focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" />

                <div className="flex items-center mt-5">
                  <input id="retirementCheckbox" type="checkbox" value="" checked={useRegularDepositEndDateChange} onChange={handleUseRegularDepositEndDateChange} className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" />
                  <label htmlFor="retirementCheckbox" className="ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">Use Regular Deposit End Date (Retirement)</label>
                </div>

                {useRegularDepositEndDateChange && (
                  <div>
                    <label htmlFor="regularDepositEndDatePicker" className="block text-md font-semibold mt-3">Regular Deposit End Date</label>
                    <Datepicker id="regularDepositEndDatePicker" className="mt-2" defaultValue={new Date()} showClearButton={false} minDate={new Date()} value={regularDepositEndDate ?? new Date()} onChange={(date) => setRegularDepositEndDate(Array.isArray(date) ? date[0] : (date as Date))} />
                  </div>
                )}
              </div>
            </div>
